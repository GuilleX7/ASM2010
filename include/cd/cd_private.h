...
struct _cdCanvas
{
  ...
  /* all the following function pointers can be 0 */
  ...
  cdfRect clip_frect;
  int clip_poly_n;
  cdPoint* clip_poly;    /* only defined if integer polygon created, if exist clip_fpoly is 0, and ->Poly exists */
  cdfPoint* clip_fpoly;  /* only defined if real polygon created, if exist clip_poly is 0, and ->fPoly exists  */
...
#define _cdCheckCanvas(_canvas) (_canvas!=0 && ((unsigned char*)_canvas)[0] == 'C' && ((unsigned char*)_canvas)[1] == 'D')
